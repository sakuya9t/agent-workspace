...
persistence failure, async boundaries and operability); **status reconcile
2026-07-24**, against `release/next` @ `02792b9` (sequencing, shipped REC/fork,
growth triggers and test-inventory drift). Status: the
...
(SEC-2, MVP-RICH, V-track and packaging) and the product surfaces added since
this review (fork, branch management, uploads, Agent Deck and managed UI).
Every RF item is scoped as
...
| ~~architecture.md named yamux as relay framing default~~ | backlog → DOC-1 | ✅ corrected 2026-07-24 |
...
| ~~Latent defects: `pull` credential-prompt hang; adopt→reconnect cursor-0 rewind; worktree created before create-validation; silent ring-alloc output drop; `exit_signal` never populated; fabricated 24×80 adopt fallback; server/client disagreement over whether `indeterminate` is terminal; invalid `ASM_BACKEND` silently selects non-durable native mode~~ | §6.1 | ✅ FIX (2026-07-25): all eight, each with a regression test |
| Client↔daemon WS contract implicit + unversioned; constants hand-mirrored (`ws.rs:21` ↔ `Terminal.tsx:19`); server→client has no frame type at all | §6.3 | RF-WSPROTO |
| Client lint gate is one i18n rule (no react-hooks, no tseslint recommended); no CI; no router/AppState or WS-attachment harness (one path-classifier unit exists); `Holder` trait has no mock; M4-C holder RPCs kept warm by nothing | §6.4 | RF-GATE |
| `generated.rs` regenerated by hand; protocol doc now says so; no schema↔generated drift check | §6.4 #6 | RF-GATE |
...
7. **RF-LIFE** (§7.2) — state transitions, atomicity and compensation. **REC
   already shipped as fork** and is no longer downstream of this work.
8. **SEC-2 + RF-ERR** together (RF-ERR pairs with §6.2's typed
   `run_blocking`). SEC-1(direct) is decided; relay encryption belongs to R5.
...
12. **RF-REC** (§6.2, legacy id) opportunistically with RF-ERR/RF-QUERY or the
    next Git/SCM change. It is generic consolidation, not a recovery gate.
13. **RF-HYG** (§6.5) opportunistically; its `MonitorState` item **before
    MEAS**. The Terminal.tsx trigger has fired (1,087 lines after more terminal
    features), so split it before another substantial feature or xterm upgrade.
14. RF-VT100 on trigger events (unchanged).
...
default-to-no-capability shape was validated by the shipped fork seam). The new debt
is not uniform. It concentrates in four places:

1. the **create/adopt/teardown lifecycle + the non-transactional metadata DB**
   — fork/recovery shipped through these paths, but the underlying debt remains
   (→ FIX, RF-LIFE; RF-REC now owns only generic consolidation);
...
3. the **build gate**, which is much thinner than it looks — client lint is a
   single i18n rule, there is no CI, the `api/` layer has one pure path test but
   no router/WS harness, and the M4-C holder RPCs are kept warm by nothing
   (→ RF-GATE);
4. **eight latent defects** that pre-date this review (→ FIX).

### 6.1 FIX — verified latent defects (P1) ✅ landed 2026-07-25

All verified by hand. Small individually (~2–3 days total including regression
tests); listed most severe first.

**All eight are fixed**, each with a test that fails without its fix. What
shipped, in the order below: (1) a single `git_command` builder owns
`GIT_TERMINAL_PROMPT=0` for every git child, with a test asserting nothing
bypasses it; (2) `Holder::route` takes the cursor its attach starts from, so a
drop before the first post-adopt chunk resumes exactly instead of replaying the
ring — and the drain loop's duplicate tracker is gone, replaced by
`stream_cursor`; (3) `create_session` validates before `resolve_workspace` and
rolls the instance back on any later failure, including a failed spawn (whose
session row still survives as `failed`); (4) a dropped ring push is logged,
counted, and delivered to the attacher as `ALLOC_FAILED`; (5) asmux reaps with
`waitpid`, so `exit_signal` carries the real signal; (6) a session row that
vanishes mid-reconcile is logged and skipped, never adopted on invented
geometry; (7) `SessionStatus::is_definitively_ended` — which excludes
`indeterminate` — now gates archive, worktree cleanup and workspace unregister,
matching what the client already assumed; (8) an unknown `ASM_BACKEND` value is
rejected at startup.
...
5. **`exit_signal` is never populated.** Both exit arms record `signal: 0`
   ...
   (`asmux.fbs`) promises `exit_signal != 0` ⇒ signalled. Lifecycle and
   ...
   reserved-until-M5 in the schema.
...
### 6.2 RF-REC — generic Git/API/UI consolidation (legacy id; P2)
...
it protects every caller rather than being framed as recovery prep. REC then
shipped as the more general fork feature on 2026-07-14 without this bundle.
What remains is still useful shared structure, but it no longer gates recovery.
Keep the legacy id to avoid reference churn. ~2–3 days at the current size.
...
2. **`require_session` + typed `run_blocking`.** The get-session-or-404
   ...
   Pairs with RF-ERR; the shipped fork/branch/deck/upload endpoints demonstrate
   that each new surface otherwise clones whatever spine exists.
3. **Client: split `ScmPanel` out of `RightPanel`.** `RightPanel.tsx` (1,209
   lines on 2026-07-24) fuses the SCM subsystem — 5 queries / 6 SCM mutations
   plus cleanup,
   past RF-QUERY's "5-query/3-mutation" description — with the VS Code
   launcher, metadata, cleanup, upload, commit action and summary blocks.
   MVP-RICH will clone whatever shape exists. Split
   `ScmPanel` / `SessionMeta` / `VscodeLauncher` (pure prop-drill of
   `target`+`session`) before adding another right-panel subsystem.

REC status: the default-capability plugin seam, SCHEMA_V7 identity capture,
native Claude/Codex fork and fallback digest all shipped in `3917f69`.
The remaining provider work is tracked separately as FORK-SHELL/FORK-OC; it is
...
in code — `CLOSE_SUPERSEDED = 4001` (`api/ws.rs:21`), the `{"t":"i"|"r"}`
client frames, a binary-first-frame snapshot convention, and two out-of-band
text sentinels (`api/ws.rs:313,320`) — and the client re-hardcodes the same
constants independently (`Terminal.tsx:19`, frame literals including
`:180,:337,:403`, plus literal sites across client + e2e scripts).
The receiving path treats **every** server→client message as terminal bytes
(`Terminal.tsx:362-366`), so a control message has literally nowhere to land
...
1. **Client lint is a single rule.** `eslint.config.js` enforces only
   ...
   already known: `Terminal.tsx:1001`'s dep array omits `target.relayKey`
...
7. **Migration ladder + version-skew guards.** No test opens an old-version
   ...
   (`domain.rs:35-45`) — silently conflating an unknown future state with a real
   failure. Reject
...
- **`attach_or_resync` consolidation**: the Ok/Gap/Conn/Code dance is
  ...
  `asmux_client.rs:424-437`), each subtly different; M4-C `readBuffer` or the
  next holder resync path would clone another.
...
- ~~Stale `yamux` reference in the frozen-contract module's own doc comment~~
  — corrected with DOC-1 on 2026-07-24.
...
- **`TerminalHeader` extraction**: the terminal-header + `UsageModal` block
  ...
  shells free".
- **Terminal.tsx split** (~1.5–2 days — the largest item, trigger fired):
  `Terminal.tsx` reached 1,087 lines by 2026-07-24; its single effect spans
  roughly lines 198–1001 and mixes xterm setup, three private-`_core` monkeypatches
  ...
  a large touch-gesture state machine, tablet controls and attachment handling.
  Extract `attachTouchGestures` /
  ...
  upgrade fails loudly. Do this before the next significant terminal feature or
  any xterm bump.
...
ordered-teardown bullets. RF-REC now retains only the shared Git/API/UI
consolidation under its legacy id.
...
- Verification refresh (2026-07-24): all 256 Rust tests pass. The client build
  could not be re-run in this checkout because dependencies are not installed
  (`tsc` is absent). CI remains the authoritative fix for machine-dependent
